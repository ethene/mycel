#!/usr/bin/env python3
"""
Generate PNG notification icons from Android Vector XML at different densities.
Fixed version with proper namespace handling.
"""
...
'''
        ...
        # Find all path elements (both direct and in groups)
        for child in root:
            if child.tag == 'path':  # Direct path
            ...
            elif child.tag == 'group':  # Group with transformations
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python generate-notification-pngs-fixed.py <vector_xml_file> <output_directory> [icon_name]")
        sys.exit(1)
