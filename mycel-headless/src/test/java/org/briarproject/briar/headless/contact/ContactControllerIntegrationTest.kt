...
import com.quantumresearch.mycel.spore.api.contact.HandshakeLinkConstants.BASE32_LINK_BYTES
import com.quantumresearch.mycel.app.headless.IntegrationTest
import com.quantumresearch.mycel.app.headless.url
import com.quantumresearch.mycel.app.test.BriarTestUtils.getRealHandshakeLink
...
class ContactControllerIntegrationTest : IntegrationTest() {
    ...
    fun `returns own handshake link`() {
        ...
        assertTrue(link.startsWith("mycel://"))
    ...
    fun `adding a pending contact with invalid public key`() {
        val alias = "AliasFoo"
        val json = """{
            "link": "mycel://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "alias": "$alias"
        }"""
        val response = post("$url/contacts/add/pending", json)
        assertEquals(400, response.code)
